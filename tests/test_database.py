...
class TestDatabase(unittest.TestCase):
    ...
    def test_table_creation(self):
        ...
        for table in self.tables_names:
            select_str = (f"SELECT count(*) FROM sqlite_master WHERE"
                          f" type='table' AND name='{table}';")
            self.assertTrue(self.alt_conn.execute(select_str).fetchone()[0])

    def test_valid_add(self):
        """Test guitarists and bands are added to
        the database successfully by the DatabaseManager"""
        self.test_db.add_guitarist_and_bands('guitarist1', ['band1', 'band2'])
        self.test_db.add_guitarist_and_bands('guitarist2', 'band3')

        self.assertTrue(
            self.alt_conn.execute(
                (f"SELECT COUNT(*) FROM {self.tables_names[0]} "
                 f"WHERE full_name='guitarist1'")
            ).fetchone()[0]
        )
        self.assertTrue(
            self.alt_conn.execute(
                (f"SELECT COUNT(*) FROM {self.tables_names[0]} "
                 f"WHERE full_name='guitarist2'")
            ).fetchone()[0]
        )
        self.assertTrue(
            self.alt_conn.execute(
                (f"SELECT COUNT(*) FROM {self.tables_names[1]} "
                 f"WHERE name='band1'")
            ).fetchone()[0]
        )
        self.assertTrue(
        ...
        self.assertTrue(
            self.alt_conn.execute(
                (f"SELECT COUNT(*) FROM {self.tables_names[2]} WHERE "
                 f"guitarist_id=1 AND band_id=1")
            ).fetchone()[0]
        )
        self.assertTrue(
            self.alt_conn.execute(
                (f"SELECT COUNT(*) FROM {self.tables_names[2]} WHERE "
                 f"guitarist_id=1 AND band_id=2")
            ).fetchone()[0]
        )
        self.assertTrue(
            self.alt_conn.execute(
                (f"SELECT COUNT(*) FROM {self.tables_names[2]} WHERE "
                 f"guitarist_id=2 AND band_id=3")
            ).fetchone()[0]
        )

    def test_invalid_add(self):
        """Test invalid guitarists and bands are not added to
        the database successfully by the DatabaseManager"""
    ...
    def test_valid_get(self):
        """Test guitarists and bands are retrieved from
        the database successfully by the DatabaseManager"""
